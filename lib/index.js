...
const DEFAULT_IGNORE_LABELS = "nocombine";
const DEFAULT_INCLUDE_LABELS = "";
const DEFAULT_OPEN_PR = true;
const DEFAULT_PR_TITLE = "Update combined dependencies";
 ...
 * @param {string} [options.ignoreLabels]
 * @param {string} [options.includeLabels]
...
const combinePRs = async (
  { github, logger, target },
  {
    ...
    ignoreLabels = DEFAULT_IGNORE_LABELS,
  ...
  for (const pull of pulls) {
    ...
    if (mustBeGreen) {
      ...
      const isNotAllGreenOrSkipped = (check) => {
        ...
        const isAllGreenOrSkipped = !allowSkipped
          ? isSuccessfull
          : isSuccessfull || isSkipped;
    ...
    let explodedIgnoreLabels = ignoreLabels.split(",");

    if (
      explodedIgnoreLabels &&
      explodedIgnoreLabels.length > 0 &&
      pull.labels.some(
        (label) =>
          explodedIgnoreLabels.find(
            (ignoredLabel) => ignoredLabel === label.name
          ) !== undefined
      )
    ) {
      logger.warning(`${ref} has labels ${ignoreLabels}. Not combining.`);
      continue;
    }

    let explodedIncludeLabels = includeLabels.split(",");

    if (
      explodedIncludeLabels &&
      explodedIncludeLabels.length > 0 &&
      !explodedIncludeLabels.every(
        (includeLabel) =>
          pull.labels.find((label) => includeLabel === label.name) !== undefined
      )
    ) {
      logger.warning(`${ref} doesn't have labels ${includeLabels}. Skipping.`);
      continue;
    }
